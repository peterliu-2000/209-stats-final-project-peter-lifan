...
linear regression way to giving point estimation and CI for Lin's estimator
...
**TODO: a bit more research detail on mechanism: how this works?**

More robust SE, clustering by user_id

```{r}
library(sandwich)
library(lmtest)

# Cluster-robust covariance by user
vcov_user <- vcovCL(lin_click, cluster = ~ user_id)

coeftest(lin_click, vcov = vcov_user)["T", ]
coeftest(lin_like,  vcov = vcov_user)["T", ]
```

After controlling for covariates and potential interaction effects, we can see that the previously identified negative relationship between horizontal video format and video click through rate vanishes: i.e. when adjusting for video duration, user features, video type, and treatment–covariate interactions, there is **no evidence** of a causal effect of horizontal format on click-through probability.
...
```{r}
#clicks estimate:
lin_click_27k <- lm(form_click, data = centered_27k)
summary(lin_click_27k)$coefficients["T", ]
```

```{r}
#like estimate
lin_like_27k <- lm(form_like, data = centered_27k)
summary(lin_like_27k)$coefficients["T", ]
```
...
# Cluster-robust covariance by user
vcov_user <- vcovCL(lin_click_27k, cluster = ~ user_id)

coeftest(lin_click_27k, vcov = vcov_user)["T", ]
coeftest(lin_like_27k,  vcov = vcov_user)["T", ]
```

Again, no significant relationship identified for both click through and like ratio using 27K data, suggesting data scarcity was NOT the cause for large Confidence Interval
...
```{r}
standard_merged$T <- as.integer(standard_merged$server_width > standard_merged$server_height)
...
The original observational studies contains 6.6M entries. For computational efficiency, we have downsized it by a factor of 20 for the remaining analysis
...
ps_formula <- as.formula(
  paste("T ~", paste(all_covariates, collapse = " + "))
)
...
#### IPW: Inversely Propensity Weighting
...
Note that even after IPW, video duration is still not balanced in control vs treatment (**suggesting the logistic propensity model misspecified for relationship between video format and video duration?)**. Empirical distributions of video duration under horizontal vs vertical video are very different:

```{r}
standard_under10 <- standard_cc %>%
  filter(video_duration <= 10 * 60 * 1000)

library(ggplot2)

ggplot(standard_under10, aes(x = video_duration, fill = factor(T))) +
  geom_density(alpha = 0.4) +
  scale_fill_manual(values = c("0" = "skyblue", "1" = "orange"),
                    labels = c("Vertical (T = 0)", "Horizontal (T = 1)")) +
  labs(
    title = "Video Duration Distribution (Under 15 Minutes)",
    x = "Video Duration (milliseconds)",
    y = "Density",
    fill = "Video Format"
  ) +
  coord_cartesian(xlim = c(0, 10 * 60 * 1000)) +
  theme_minimal()
...
**havent quite figured out this part yet!**
...
m.out <- matchit(
  formula = ps_formula,
  data = standard_ipw,
  distance = standard_ipw$ps,   # use your own PS model
  method = "nearest",           # nearest neighbor
  replace = FALSE,              # no replacement (optional)
  caliper = 0.05                # restrict matches to similar PS
)
```
